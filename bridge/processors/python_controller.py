...
class MatlabController(BaseProcessor):
    ...
    def process(self) -> None:
        ...
        if ssl_package:
            ...
            if dbg:
                self.field.b_team[3].go_to_point_vector_field(auxiliary.Point(1000, 1000), self.field)
                self.field.b_team[1].go_to_point_vector_field(auxiliary.Point(700, 1300), self.field)
                self.field.b_team[3].rotate_to_angle(math.pi)
                self.field.b_team[1].rotate_to_angle(0)
            else:
                self.field.b_team[3].go_to_point_vector_field(auxiliary.Point(-1000, -1000), self.field)
                self.field.b_team[1].go_to_point_vector_field(auxiliary.Point(-700, -1300), self.field)
