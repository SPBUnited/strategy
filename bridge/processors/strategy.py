...
class Strategy:
    ...
    def attack(self, field: field.Field):
        rivals = field.b_team
        allies = field.y_team
        waypoints = [None]*const.TEAM_ROBOTS_MAX_COUNT
        ...
        goal_points = [aux.Point(-4500, i) for i in range(-500, 501)]
        bro_points = [allies[i].getPos() for i in range(len(allies))]
        # print(bro_points)
        if self.robot_with_ball != None:
            if self.attack_state == "TO_BALL":
                self.attack_pos = field.ball.pos
                if aux.in_place(self.attack_pos, allies[self.robot_with_ball].getPos(), 500):
                    self.attack_state = "CALCULATING"
            elif self.attack_state == "CALCULATING":
                shot_pos, shot_prob = aux.shotDecision(field.ball.pos, goal_points, rivals[:2])
                ...
                # print(shot_pos)
                if aux.in_place(self.attack_pos, allies[self.robot_with_ball].getPos(), 50):
                ...
                # if shot_prob > const.KOEFF_NAGLO:
                # self.attack_pos = shot_pos
                allies[self.robot_with_ball].kickerChargeEnable = 1
                allies[self.robot_with_ball].kickerVoltage = 15
                if aux.in_place(self.attack_pos, allies[self.robot_with_ball].getPos(), 50):
                ...
                allies[self.robot_with_ball].kickerChargeEnable = 1
                allies[self.robot_with_ball].kickerVoltage = 15
                allies[self.robot_with_ball].kick_up()
                if not(aux.in_place(self.attack_pos, allies[self.robot_with_ball].getPos(), 500)):
                    self.attack_state = "TO_BALL"
                #ехать к нужной точке
            waypoints[self.robot_with_ball] = wp.Waypoint(self.attack_pos, aux.angle_to_point(allies[self.robot_with_ball].getPos(), field.ball.getPos()), wp.WType.ENDPOINT)
