...
class Strategy:
            ...
            if self.game_status == GameStates.TIMEOUT:
                ...
                for i in range(const.TEAM_ROBOTS_MAX_COUNT):
                    if field.allies[i].is_used():
                        waypoint = wp.Waypoint(aux.Point(2000 * const.SIDE, 1250 - 500 * rC), field.allies[i].getAngle(), wp.WType.ENDPOINT)
                        waypoints[i] = waypoint
                        rC+=1
    ...
    def defence(self, field: field.Field, waypoints):
        ...
        if totalRobots - len(used_robots_id) == 1:
            ...
            def2 = aux.find_nearest_robot(aux.Point(const.SIDE * -xAttack, 1500), field.allies, used_robots_id)
    ...
    def attack(self, field: field.Field, waypoints):
            ...
            if self.attack_state == "TO_BALL":
                ...
                if not(aux.in_place(self.attack_pos, field.allies[self.robot_with_ball].getPos(), 500)):
                    # for index in range(len(used_pop_pos)):
                        ...
                        #     waypoints[self.robot_with_ball] = wp.Waypoint(popusk_positions[index], 0, wp.WType.ENDPOINT)
        ...
        for robot in self.popusk:
            ...
            waypoints[robot] = wp.Waypoint(popusk_positions[pop_pos], 0, wp.WType.ENDPOINT)